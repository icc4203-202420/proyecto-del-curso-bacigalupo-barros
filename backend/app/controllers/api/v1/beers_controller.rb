  ...
  def show
    ...
    if @beer.image.attached?
      render json: {
        ...
        brewery: @brewery.as_json, #probando para mostrar las breweries de las cervezas
        bars: @bars.as_json(only: [:id, :name, :address]) #probando para mostrar los bares de las cervezas
      }, status: :ok
    else
      render json: {
        beer: @beer.as_json,
